#!/usr/bin/env python3
"""
Create PDF document from PPT slides.
"""
...
from reportlab.platypus.flowables import Table, TableStyle
from reportlab.lib.enums import TA_CENTER, TA_LEFT

# Configuration
IMAGES_DIR = "/Volumes/My house/Users/Sheldon/.openclaw/workspace/xiao-mi/ppt_images"
OUTPUT_PDF = "/Volumes/My house/Users/Sheldon/.openclaw/workspace/xiao-mi/OpenClaw_云端部署与培养指南.pdf"

# Slide content (simplified for PDF)
SLIDES = [
    {
...
def create_pdf():
    ...
    for i, slide in enumerate(SLIDES):
        ...
        if os.path.exists(image_path):
            ...
            # Scale image to fit page width (landscape A4)
            max_width = landscape(A4)[0] - 60  # 60pt margin on each side
            scale = max_width / img_width if img_width > max_width else 1.0
            display_width = img_width * scale
            display_height = img_height * scale
            
            # Add image to document
            doc.build([
                Paragraph(slide['title'], title_style),
                Spacer(1, 6),
                RLImage(
                    image_path,
                    width=display_width,
            ...
            if 'content' in slide and slide['content']:
                ...
                for line in slide['content']:
                    doc.build([
                        Paragraph(line, content_style),
                    ])
            
            # Page break for each slide (except last)
            if i < len(SLIDES) - 1:
                doc.build([PageBreak()])
