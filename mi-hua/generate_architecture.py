...
try:
    ...
    font_medium = ImageFont.truetype("/System/Library/Fonts/PingFang.ttc", 20)
...
# 标题
title = "Molili 架构流程"
draw.text((width//2 - 100, 40), title, fill=text_dark, font=font_large)
...
# 文字
draw.text((wechat_x - 25, node_y + 25), "微信", fill=text_dark, font=font_medium)
draw.text((wechat_x - 35, node_y + 50), "用户消息", fill=text_gray, font=font_small)
...
draw.text((molili_x - 45, node_y + 50), "消息处理", fill=text_gray, font=font_small)
...
# 文字
draw.text((ai_x - 35, node_y + 25), "AI模型", fill=text_dark, font=font_medium)
draw.text((ai_x - 45, node_y + 50), "智能回复", fill=text_gray, font=font_small)
...
# 流程说明文字
draw.text((wechat_x + 80, arrow_y - 25), "转发", fill=tech_blue, font=font_small)
draw.text((molili_x + 80, arrow_y - 25), "处理", fill=ai_purple, font=font_small)
draw.text((width//2 - 30, return_y + 10), "返回回复", fill=text_gray, font=font_small)
...
for i, feature in enumerate(features):
    draw.text((width//2 - 80, 450 + i * 35), feature, fill=text_dark, font=font_medium)
